...
def svm_loss_vectorized(
    ...
    # result in loss.                                                           #
    #############################################################################
    # Compute a scores matrix of shape (N,C) for each sample where each column 
    # corresponds to the class score of that sample
    scores = X.mm(W)
    
    # Compute margin value for each class value of each sample
    margin = torch.clamp(scores - scores.gather(1, y.view(-1,1)) + 1, min=0)

    # Set correct class margin values to zero to prevent its contribution to loss computation
    margin[torch.arange(X.shape[0]), y] = 0
    
    # Compute loss
    loss = torch.mean(torch.sum(margin, dim=1))
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    #############################################################################
    dloss = 1/X.shape[0]
    dmargins = (margin>0) * dloss
    dscores = dmargins
    dscores[torch.arange(X.shape[0]), y] -= torch.sum(dscores, dim=1)
    dW = X.t().double().mm(dscores.double())
    
    
    # Computed the gradient of the loss function w.r.t. the regularization term
    # and added it to dW
    dW += 2 * reg * W   
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def svm_get_search_params():
    ...
    ###########################################################################
    learning_rates = [8e-3,1e-2, 2e-2, 3e-2]
    regularization_strengths = [8e-3, 9e-3,1e-2, 2e-2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    _ = cls.train(X_train= data_dict['X_train'], y_train= data_dict['y_train'],
    ...
    train_acc = 1 - (torch.count_nonzero(data_dict['y_train'] - y_pred_train) / len(y_pred_train))
    val_acc = 1 - (torch.count_nonzero(data_dict['y_val'] - y_pred_val) / len(y_pred_val))
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
